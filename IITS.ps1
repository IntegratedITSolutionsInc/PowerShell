...
<#
.Synopsis
   Gives user full access rights to another's mailbox BUT removes automap
.DESCRIPTION
...
#>
function mailbox-rights-no-mapping
{
    [CmdletBinding()]
    [Alias()]
    [OutputType([int])]
    ...
    (
        ...
        [Parameter(Mandatory=$true,
                   ValueFromPipelineByPropertyName=$true,
                   Position=0)]
        Param ([string]$owner=(read-host "Enter mailbox owner's email address:"))
        Param ([string]$user=(read-host "Enter user getting access email address:"))
    )
    ...
    Add-MailboxPermission -Identity $owner -User $user -AccessRights FullAccess -AutoMapping:$false
